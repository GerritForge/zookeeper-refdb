...
public class ZkInit implements InitStep {
  ...
  ZkInit(ConsoleUI ui, SitePaths site, @PluginName String pluginName, Injector initInjector)
    ...
    secureConfig = new FileBasedConfig(site.secure_config.toFile(), FS.DETECTED);
    secureConfig.load();
    config =
  ...
  public void run() throws Exception {
    ...
    try {
      try (CuratorFramework curator = new ZookeeperConfig(secureConfig, config).startCurator()) {
