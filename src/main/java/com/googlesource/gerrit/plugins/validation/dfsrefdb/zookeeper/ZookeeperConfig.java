  ...
  public CuratorFramework buildCurator() {
    ...
    if (build == null) {
      ...
      if ((zkUsername.isPresent() && !zkPassword.isPresent())
          || (!zkUsername.isPresent() && zkPassword.isPresent())) {
        throw new IllegalArgumentException(
            "Only one between password or username for Zookeeper was set, please set both to succesfully authenticate");
      } else {
        zkUsername.ifPresent(
            usr -> zkPassword.ifPresent(pwd -> configureAuth(builder, usr + ":" + pwd)));
      }
