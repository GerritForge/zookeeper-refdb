pipeline {
    ...
    stages {
        ...
        stage('Copyright') {
            steps {
                script {
                    def formatOut = sh (script: "cd zookeeper-refdb && find . -type f -name '*.java' -exec awk '!/Copyright \\(C\\) [[:digit:]][[:digit:]][[:digit:]][[:digit:]] The Android Open Source Project/ {print FILENAME} {nextfile}' {} +", returnStdout: true)
                    if (formatOut.trim()) {
                        def files = formatOut.split('\n').collect { it.split(' ').last() }
    ...
    post {
        success { when { ${env.GERRIT_CHANGE_NUMBER} { gerritReview labels: [Verified: 1] } } }
        unstable { when { ${env.GERRIT_CHANGE_NUMBER} { gerritReview labels: [Verified: 0], message: "Build is unstable: ${env.BUILD_URL}" } } }
        failure { when { ${env.GERRIT_CHANGE_NUMBER} { gerritReview labels: [Verified: -1], message: "Build failed: ${env.BUILD_URL}" } } }
    }
}
